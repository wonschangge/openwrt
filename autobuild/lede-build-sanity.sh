...
prepare_opensync() {
	for pch in autobuild/$1/patch_for_makefile/*; do patch -p1 < $pch; done
	cp -rfa ${BUILD_DIR}/../app/opensync/device-vendor-plume-openwrt/ ${BUILD_DIR}/../
	cp -rfa ${BUILD_DIR}/../app/opensync/device-platform-cfg80211/ ${BUILD_DIR}/../
}
...
build() {
	...
	echo "make V=1 -j2 download world"
	make V=1 -j2 download world || exit 1
